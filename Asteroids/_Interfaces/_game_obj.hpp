...
color color{};
SDL_Color player_color = { color.r, color.g, color.b, color.a };
...
{
public:
	game_obj(SDL_Point *_vertices, const int &_num_pts, SDL_Point _position, game_obj_velocity _av, bool _on_off) :
...
public:
	...
	void move() {
		...
		// this is a tricky bit as SDL_points are in ints and so very small incremental movement doesn't necessarily show correctly. object movment is tracked using floating point precision but then incremented by rounding the floating points when required.
		if (fabs_x >= 1.0) {
	...
	bool collider(const SDL_Point &point) {
